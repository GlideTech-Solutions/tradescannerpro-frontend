...
class ApiClient {
  ...
  async scanCryptoByCategory(category) {
    return this.get(`/crypto/scan?category=${category}`);
  }

  async getNeutralPicks() {
    console.log("apiClient.getNeutralPicks() called");
    const result = await this.get('/crypto/scan?category=neutral');
    console.log("apiClient.getNeutralPicks() result:", result);
    return result;
  }

  async getBullishPicks() {
    return this.get('/crypto/scan?category=strong_bullish');
  }

  async getBearishPicks() {
    return this.get('/crypto/scan?category=strong_bearish');
  }
